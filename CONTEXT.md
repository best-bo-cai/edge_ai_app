...
外部 API 请求的流水记录（时间、端点、模型、状态码、token 数、来源 IP、请求与响应内容）。API 请求不产生会话，只记入调用日志，在独立的日志列表页查看，可整表清空、不做自动清理。

## 调用统计（Call Statistics）

API 调用日志的聚合视图：总调用数与失败数。失败指状态码 ≥ 400 的调用；状态码未知的存量记录不计入失败。统计始终由日志聚合而来，两者永远一致，统计卡片点击即进入日志列表页。
